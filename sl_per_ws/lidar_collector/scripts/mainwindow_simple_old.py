...
- Left LiDAR: 47MDM7A0020002
- Right LiDAR: 47MDM5E0020051
...
class DualLidarInterface:
    ...
    def run_auto_pipeline(self):
        ...
        try:
            ...
            self.update_progress(self.pipeline_progress, "error", str(e))
    ...
    def _step5_target(self):
        """Step 5: Target path generation (paview1.py)"""
        try:
            ...
            self.write_log("Warning: paview1.py timed out")
        except Exception as e:
...
def api_target_data():
    """Get target path data as JSON for Three.js rendering (paview1.py style)

    Returns path data with:
    - Positions: All point positions
    - Directions: Normal/direction vectors for each point
    - Lines: Array of line data with start/end indices for drawing connected lines
    """
    try:
        iktarget_dir = os.path.join(WORKSPACE_ROOT, "iktarget")
        ...
        # Extract all points from all lines with line indices (like paview1.py)
        all_points = []
        all_directions = []
        ...
        for line in data.get('lines', []):
            ...
            for pt in line:
                # Point format: [x, y, z, nx, ny, nz, ...]
                if len(pt) >= 6:
                    all_points.append(pt[:3])  # position
                    all_directions.append(pt[3:6])  # direction
