...
public:
    ...
    {
        // Declare parameters
        this->declare_parameter<std::string>("left_topic", "/livox/lidar_192_168_1_102");
        this->declare_parameter<std::string>("right_topic", "/livox/lidar_192_168_1_103");
        ...
        RCLCPP_INFO(this->get_logger(), "Dual LiDAR Node Initialized");
        RCLCPP_INFO(this->get_logger(), "========================================");
...
private:
    ...
    void leftLidarCallback(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
    {
        ...
        if (!temp_cloud.empty()) {
            ...
            for (const auto& pt : temp_cloud) {
                left_cloud_->push_back(convertPoint(pt, false));
            }
            RCLCPP_DEBUG(this->get_logger(), "[LEFT] Received %zu points, Total: %zu",
                temp_cloud.size(), left_cloud_->size());
        }
    }
    ...
    void rightLidarCallback(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
    {
        ...
        if (!temp_cloud.empty()) {
            ...
            for (const auto& pt : temp_cloud) {
                right_cloud_->push_back(convertPoint(pt, correct_right_yaw_));
            }
            RCLCPP_DEBUG(this->get_logger(), "[RIGHT] Received %zu points, Total: %zu",
                temp_cloud.size(), right_cloud_->size());
        }
    }
