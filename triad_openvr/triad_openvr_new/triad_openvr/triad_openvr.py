...
def convert_to_euler(pose_mat):
    """
    Convert OpenVR pose matrix to Unity-compatible Euler angles.
    Unity uses ZXY rotation order by default.
    Includes OpenVR (right-handed) to Unity (left-handed) coordinate system conversion.
    """
    # Extract position
    x = pose_mat[0][3]
    y = pose_mat[1][3]
    ...
    # Apply OpenVR to Unity coordinate conversion for position
    # Unity uses left-handed coordinate system, OpenVR uses right-handed
    # Z-axis needs to be inverted
    z = -z
    
    # Extract Euler angles from rotation matrix using ZXY order
    # This matches Unity's default rotation order
    ...
    if abs(cos_pitch) > 0.001:  # Not in gimbal lock
    ...
    # Apply additional rotation corrections for coordinate system differences
    # You may need to adjust these based on testing
    # yaw = -yaw  # Uncomment if Y-axis rotation needs to be inverted
    
    # Return in standard order: position (x,y,z) and rotation (pitch, yaw, roll)
