...
def data_publisher(q, a):
    prev_time = datetime.now().second
    rclpy.init()
    msg = std_msgs.String()
    print('ros2 initialized')
    
    node = rclpy.create_node('current_pose')
    pose_pub = node.create_publisher(std_msgs.String,"/current_pose", 1)
    
    while True:
            
        q.put('next')
        
        #tf_broadcasting(msg.data)

        time.sleep(0.03)
    
#p = Process(target=data_publisher, args=(q, a))   
#p.start()    
...
def ConnectToCB(ip):
    ...
    print(cmd_connect, data_connect)
...
def GetCurreJP():
    if not data_connect:
        ...
        current_joint_ = (systemstat_global.jnt_ref[0], systemstat_global.jnt_ref[1], systemstat_global.jnt_ref[2]
                               , systemstat_global.jnt_ref[3], systemstat_global.jnt_ref[4],
                               systemstat_global.jnt_ref[5])
        current_pose = str(current_tcp_) + ';' + str(current_joint_) + ';' + str(systemstat_global.robot_state)
        return current_pose
    else:
        print("'data socket isn't connect")
        current_tcp_ = (0, 0, 0, 0, 0, 0)
        current_joint_ = (0, 0, 0, 0, 0, 0)
        current_pose = str(current_tcp_) + ';' + str(current_joint_) + ';' + str(systemstat_global.robot_state)
        return current_pose
...
def MoveL(x, y, z, rx, ry, rz, spd, acc):
    msg = 'move_l(pnt[' + str(x) + ',' + str(y) + ',' + str(z) + ',' + str(rx) + ',' + str(ry) + ',' + str(
        rz) + '], ' + str(spd) + ',' + str(acc) + ')'
    q.put([0, float(x), float(y), float(z)])
    return SendCOMMAND(msg, CMD_TYPE.MOVE)
...
def ServoJ(x, y, z, rx, ry, rz, t1, t2, gain, alpha):
    msg = 'move_servo_j(jnt[' + str(x) + ',' + str(y) + ',' + str(z) + ',' + str(rx) + ',' + str(
        ry) + ',' + str(rz) + '], ' + str(t1) + ',' + str(t2) + ',' + str(gain) + ',' + str(alpha) +')'
    #print(msg)
    return SendCOMMAND(msg, CMD_TYPE.MOVE)
    
@dispatch(float, float, float, float, float, float, float, float, float, float)
def ServoL(x, y, z, rx, ry, rz, t1, t2, gain, alpha):
    msg = 'move_servo_l(pnt[' + str(x) + ',' + str(y) + ',' + str(z) + ',' + str(rx) + ',' + str(
        ry) + ',' + str(rz) + '], ' + str(t1) + ',' + str(t2) + ',' + str(gain) + ',' + str(alpha) +')'
    #print(msg)
    return SendCOMMAND(msg, CMD_TYPE.MOVE)
...
@dispatch(float, float, float, float, float, float, float, int)
def MoveITPL_Add(x, y, z, rx, ry, rz, spd, cnt):
    msg = 'move_itpl_add(pnt[' + str(x) + ',' + str(y) + ',' + str(z) + ',' + str(rx) + ',' + str(ry) + ',' + str(
        rz) + '], ' + str(spd) + ')'

    if cnt == 1:
        q.put([1, float(x), float(y), float(z)])
    return SendCOMMAND(msg, CMD_TYPE.NONMOVE)


def MoveITPL_Run(acc=float, type=ITPL_RTYPE, gripper=bool):
    rtype = 0
    ...
    while True:
        ...
        if cmd_send_flag == 1:
            ...
            if msg_recv.decode('utf-8') == 'The command was executed\n':
                #print('The command was executed\n')
                cmd_send_flag = 0
                bReadCmd = True
        ...
        if msg_recv[0] == 0x24:
            ...
            if size <= len(msg_recv) - 4:
                if msg_recv[3] == 3:
                    ...
                    systemstat = systemSTAT(result[0],
                     result[1:7], result[7:13], result[13:19], 
                     result[19:25], result[25:31], 
                     result[31:35], result[35:39], result[39:55], result[55:71], 
                     result[71:77], 
                     result[77], result[78], result[79], result[80], result[81], result[82], 
                     result[83], result[84], result[85], 
                     result[86:92], result[92:98], 
                     ...
                     result[115:117], 
                     result[117:119], result[119:121], 
                     result[121], result[122], result[123], result[124], result[125], result[126])
                    """
                    if q.qsize() != 0:
                        get = q.get()
                        if get == 'next':
                            b.send(str(result[25:31])+';'+str(result[7:13])+';'+str(systemstat_global.robot_state))
                        elif get == 'run':
                            itpl_run = True
                            gripper = True
                        else:
                    ...
                    #time.sleep(0.1)
                    systemstat_global = systemstat
                    #if systemstat_global.robot_state == 1 and prev_stat == 3:
                    #    b.send(str(result[25:31])+';'+str(result[7:13]))
                    prev_stat = systemstat_global.robot_state
                    #print(math.dist(start, result[25:28]))
                    """
                    if itpl_run and gripper and math.dist(start, result[25:28]) > 50:
                        print('gripper on')
                        CBDigitalOut(0.0, DOUT_SET.HIGH)  
                        gripper = False
                    elif itpl_run and not gripper and math.dist(finish, result[25:28]) < 150:     
                        print('gripper off')
