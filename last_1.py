...
def visualize_result(stl_path, start_points, all_points, all_normals,
    ...
    if len(start_points) > 0:
        p.add_mesh(pv.PolyData(start_points), color='green', point_size=15, render_points_as_spheres=True)
    ...
    if robot_positions is not None and len(robot_positions) > 0:
        p.add_mesh(pv.PolyData(robot_positions), color='yellow', point_size=12, render_points_as_spheres=True)
        ...
        if surface_points is not None and len(surface_points) == len(robot_positions):
            ...
            for i in range(len(robot_positions)):
                ...
                line_cells.extend([2, idx, idx+1])  # 2점 연결
