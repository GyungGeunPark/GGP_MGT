...
ANGLE_ROI_TOP_LEFT = (225, 110)       # (x1, y1)
ANGLE_ROI_BOTTOM_RIGHT = (345, 220)  # (x2, y2)
ANGLE_ROI_COLOR = (0, 255, 255)      # Yellow

# Speed ROI - 속도 측정용 (대폭 확대: 프레임 전체 커버)
SPEED_ROI_TOP_LEFT = (50, 80)        # (x1, y1)
SPEED_ROI_BOTTOM_RIGHT = (225, 250)  # (x2, y2)
...
class DetectedObject:
    ...
    fp_current_yaw: float = 0.0       # 디버그: FoundationPose 현재 yaw
    fp_reference_yaw: float = 0.0     # 디버그: FoundationPose 기준 yaw
            ...
            if depth_filename and mask is not None:
                ...
                if depth_file.exists():
                    ...
                    if depth_img is not None:
                        # RGB를 BGR→RGB 변환 (FoundationPose는 RGB 입력)
...
class PersistentMultiObjectTracker:
    ...
    def _process_single_detection(
        ...
        detected_obj = DetectedObject(
            ...
            fp_reference_yaw=tracker.reference_fp_yaw if tracker.reference_fp_yaw is not None else 0.0,
...
def _run_v5_realtime(args):
    """v5 (Nimg3Pipeline + RealSenseSource) 실시간 실행 경로.

    backend='v5' 일 때 main() 이 여기로 위임한다.
    """
    import sys as _sys
    _root = Path(__file__).resolve().parents[4]     # /root/rvc_scan_ws
    ...
    from nimg_v3.input import RealSenseSource
    ...
    reco_cfg = RecognitionConfig(
        ...
        roi=(225, 110, 345, 220) if not args.no_roi else None,
    )
    ...
    if not tdb.exists():
        print(f"[v5] Template DB not found: {tdb}\n"
              f"     Run: python src/nimg_v3/scripts/build_templates.py")
        return

    # ROS2 publisher — 기존 ROS2SignalPublisher 를 옵션으로 재사용
    ros2_pub = None
    if not args.no_ros:
        try:
            ...
            print(f"[v5] ROS2 publisher on topic '{args.ros_topic}'")
        except Exception as e:
    ...
    source = RealSenseSource(width=args.width, height=args.height, fps=args.fps)
    if not source.start():
        print("[v5] RealSense start failed")
        return

    writer = None
    if args.save:
        import imageio
        writer = imageio.get_writer(args.save, fps=args.fps, codec="libx264",
                                    quality=7, macro_block_size=1)
        print(f"[v5] Saving viz to {args.save}")

    win_name = "nimg_v3 v5 — Realtime FoundationPose"
    if not args.no_display:
        cv2.namedWindow(win_name, cv2.WINDOW_NORMAL)
        cv2.resizeWindow(win_name, 960, 720)

    print("[v5] Running. 'q' to quit, 's' save snapshot, 'r' reset baseline")
    try:
        while True:
            fr = source.read()
            if fr is None:
            ...
            if not args.no_display and viz is not None:
                ...
                if key in (ord('q'), 27):
                    ...
                    for cid in list(pipe.T_ref):
                        pipe.T_ref[cid] = pipe.baseline_loader.compute_T_ref(cid)
                    print("[v5] baseline reset")
    finally:
        source.stop()
...
def main():
    ...
    # v5 통합 파이프라인 (nimg_v3.pipeline.Nimg3Pipeline)
    ...
    multi_tracker = PersistentMultiObjectTracker(
        fps=float(args.fps), intrinsics=intrinsics,
