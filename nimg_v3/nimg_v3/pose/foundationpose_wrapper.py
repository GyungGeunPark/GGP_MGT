...
class RealFoundationPoseEstimator:
    ...
    def _load_mesh(self, mesh_path: str):
        ...
        try:
            ...
            # 텍스처 이미지 없는 CAD 모델 처리: TextureVisuals인데 image가 None이면
            # vertex color로 변환 (FoundationPose make_mesh_tensors 호환)
            if isinstance(self.mesh.visual, trimesh.visual.texture.TextureVisuals):
                mat = getattr(self.mesh.visual, 'material', None)
                if mat is None or getattr(mat, 'image', None) is None:
                    logger.info("  Mesh has no texture image, converting to vertex colors")
                    # MTL diffuse 색상 또는 기본 회색
                    kd = getattr(mat, 'diffuse', None)
                    if kd is not None:
                        color = np.array(kd[:3], dtype=np.uint8)
                    else:
                        color = np.array([128, 128, 128], dtype=np.uint8)
                    self.mesh.visual = trimesh.visual.ColorVisuals(
                        mesh=self.mesh,
                        vertex_colors=np.tile(color, (len(self.mesh.vertices), 1))
                    )
    ...
    def _init_estimator(self):
        ...
        try:
            ...
            # mesh vertices를 float32로 변환 (PyTorch 2.12+ 호환)
            self.mesh.vertices = self.mesh.vertices.astype(np.float32)
            model_pts = self.mesh.vertices.copy()
            model_normals = self.mesh.vertex_normals.copy().astype(np.float32)
