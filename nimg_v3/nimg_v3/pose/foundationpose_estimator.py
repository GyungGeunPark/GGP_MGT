...
class FoundationPoseEstimator:
    ...
    def _load_neural_field(self, neural_field_dir: Optional[str]):
        ...
        if neural_field_dir is None:
            logger.warning("Neural field dir not provided, using dummy mesh")
            self._neural_field = None
            # Create dummy mesh instead
            try:
                import trimesh
                self._mesh = trimesh.creation.box(extents=[0.1, 0.1, 0.05])
                logger.info("Dummy mesh created for model-free mode")
            except ImportError:
