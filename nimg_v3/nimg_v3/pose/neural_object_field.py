...
class NeuralObjectField:
    ...
    def extract_mesh(
        ...
        우선순위:
        1. 기존 mesh.obj 파일이 있으면 로드 (TSDF Fusion으로 사전 생성된 것)
        2. 없으면 Marching Cubes로 더미 구 메시 생성

        실제 메시 생성은 scripts/generate_mesh.py를 사용하세요:
            python scripts/generate_mesh.py --data_dir <reference_images_dir>
        ...
        try:
            ...
            logger.warning(f"Mesh extraction requires trimesh: {e}")
            return None

        # 1순위: 이미 로드된 메시가 있으면 반환
        ...
        try:
            ...
            # 더미 SDF (구 형태) - 실제로는 학습된 네트워크에서 SDF 값 쿼리
            ...
            sdf = np.sqrt(xx**2 + yy**2 + zz**2) - 0.3
            ...
            logger.warning(f"Mesh extraction requires skimage: {e}")
            return None
